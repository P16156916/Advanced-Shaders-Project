...
{
	for (unsigned int i = 0; i < v_meshes.size(); i++)
		v_meshes[i].Draw(shader);
}

void Model::Draw(Shader shader, unsigned int p_uiAmount)
{
	for (unsigned int i = 0; i < v_meshes.size(); i++)
		v_meshes[i].Draw(shader, p_uiAmount);
}
...
{
	for (unsigned int i = 0; i < getMesh().size(); i++)
	{
		unsigned int VAO = getMesh()[i].getVAO();
...
Mesh Model::processMesh(aiMesh * mesh, const aiScene * scene)
{
	...
	v_vertices.resize(mesh->mNumVertices);
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		v_vertices[i] = vertices[i].position;
...
unsigned int TextureFromFile(const char* filepath, const string& directory, bool gamma)
{
